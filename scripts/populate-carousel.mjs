...
export const shellPath = new URL("../assets/database/carousel-shell.html", import.meta.url);
const escape = value => String(value).replaceAll("&", "&amp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll('"', "&quot;").replaceAll("'", "&#39;");
const template = (shell, variant) => shell.match(new RegExp(`<template data-variant="${variant}">([\\s\\S]*?)</template>`))?.[1] ?? (() => { throw new Error(`Invalid shell variant: ${variant}`); })();
...
export function populateHtml(content, shell = readFileSync(shellPath, "utf8")) {
  const count = String(content.slides.length).padStart(2, "0"), styles = shell.match(/<style>[\s\S]*?<\/style>/)?.[0] ?? (() => { throw new Error("Invalid shell"); })();
...
export function populateRun(runDirectory) { const content = validateRun(runDirectory), html = populateHtml(content); writeFileSync(join(runDirectory, "index.html"), html); return html; }
if (process.argv[1] === fileURLToPath(import.meta.url)) try { populateRun(process.argv[2] ?? ""); } catch (error) { console.error(`Populate failed: ${error.message}`); process.exitCode = 1; }
