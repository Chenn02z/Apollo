...
const text = (value, limit, field) => { validateText(value, limit); if (unbreakable(value)) fail(`${field} exceeds v2 shell token capacity (32 code points)`); };
...
function validateContent(content, topic) {
  ...
  if (!exactly(content, ["version", "topic", "slides"]) || content.version !== "2" || content.topic !== topic || !Array.isArray(content.slides) || content.slides.length < 7 || content.slides.length > 10) fail(`Invalid slide count: ${content.slides?.length ?? "unknown"}; expected 7–10`);
  content.slides.forEach(validateVariant);
}
export function validateRunV2(runDirectory) {
  const runPath = resolve(runDirectory), runId = basename(runPath), contentPath = join(runPath, "carousel-content-v2.json"); let safe = false;
  try { if (basename(dirname(runPath)) !== "runs" || !runId || lstatSync(dirname(runPath)).isSymbolicLink() || lstatSync(runPath).isSymbolicLink()) fail("Invalid run path"); safe = true; const requestPath = join(runPath, "request-v2.json"); if (lstatSync(requestPath).isSymbolicLink() || lstatSync(contentPath).isSymbolicLink()) fail("Symlinked v2 artifact"); const request = readJson(requestPath); validateRequest(request, runId); const content = readJson(contentPath); validateContent(content, request.topic); return content; }
...
if (process.argv[1] === fileURLToPath(import.meta.url)) try { validateRunV2(process.argv[2] ?? ""); } catch (error) { console.error(`Validation failed: ${error.message}`); process.exitCode = 1; }
