...
export function loadArchive() {
  ...
  try {
    ...
    if (members.join() !== "examples,preview.html,template.json" || !regular(join(archive, "preview.html")) || stat(join(archive, "examples"))?.isSymbolicLink() || !stat(join(archive, "examples"))?.isDirectory() || !exact(value, ["version", "id", "theme", "themeAssets", "motifs", "capabilities"]) || value.version !== 2 || value.id !== "database-blueprint" || value.theme !== "database" || !exact(value.themeAssets, ["css", "shell"]) || value.themeAssets.css !== "../../assets/database/theme.css" || value.themeAssets.shell !== "../../assets/database/carousel-shell.html" || !Array.isArray(value.motifs) || value.motifs.join() !== "blueprint" || !exact(value.capabilities, ["compositions", "densities", "visualAnchors", "directions"]) || Object.entries(capabilities).some(([key, expected]) => !Array.isArray(value.capabilities[key]) || value.capabilities[key].join() !== expected.join()) || !regular(assets.css) || !regular(assets.shell) || realpathSync(assets.css) !== join(root, "assets/database/theme.css") || realpathSync(assets.shell) !== join(root, "assets/database/carousel-shell.html")) fail("LAYOUT_RUN_OR_ARCHIVE", path);
