...
After the second failed candidate attempt, report `Created <run-dir>/ (review: <decision>; revision failed).` and stop.
...
If the required archive already exists, report `Created <run-dir>/ (review: <decision>; revision failed).` and stop.
