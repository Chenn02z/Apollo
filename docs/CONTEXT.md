...
- `Apollo`: the Codex-native project that turns software-engineering topics into
  self-contained, interview-ready slide decks. Under spec 0006 (Accepted, not
  yet implemented) it is entered through `$getcracked` and produces each deck
  through `$generate`.
- `$getcracked`: the sole user-facing entry point (Accepted in spec 0006, not
  yet implemented). It takes a `category` and a topic count, orchestrates
  research and generation internally, and owns the `getcracked inventory`.
  Users do not invoke research or `$generate` separately.
- `$generate`: the deck-authoring workflow, renamed from `$apollo` with no
  ...
  unchanged: it generates its own `run-id`, creates `runs/<run-id>/`, authors,
  validates, exports, and retries up to 3 attempts. Planned path
  `.agents/skills/generate/SKILL.md`; it lives at `.agents/skills/apollo/SKILL.md`
  until 0006 is implemented.
- `web-researcher`: the dedicated research agent planned at
  ...
  decisions.
- `getcracked inventory`: `docs/getcracked-inventory.md` (Accepted, not yet
  created) — the durable, checked-in backlog of topics under seven fixed
  categories, each entry carrying exactly a topic name, a status, and a run
  link.
...
- `metadata.md`: the run-scoped `runs/<run-id>/metadata.md` — a title plus a 1–2
...
- `runs/` / `run-id`: the local, gitignored generated-output directory and the
  ...
  Each new run writes its artifacts to its own folder `runs/<run-id>/` (its
  `deck.html` and `slide-01.png` … `slide-10.png`); there is no shared or cwd
  output folder. The legacy flat `runs/deck.html` is preserved as pre-0002
  evidence.
...
- `$getcracked` is the sole user-facing entry point (Accepted in spec 0006, not
  yet implemented). It takes a category and a count, uses `web-researcher` to
...
- `runs/` is local, gitignored generated output. Every deck run has a unique
  ...
  (its `deck.html` and `slide-01.png` … `slide-10.png`) live under
  `runs/<run-id>/`. Under spec 0006 (Accepted, not yet implemented) a
  successful run also carries a `$getcracked`-written `metadata.md`.
...
- No project-specific CLI or runtime command exists yet. Apollo is invoked as a
  Codex workflow, not a runnable binary. Spec 0006 (Accepted, not yet
  implemented) settles `$getcracked` at
  `.agents/skills/getcracked/SKILL.md` as the sole user-facing entry point and
  renames the deck workflow to `$generate` at
  `.agents/skills/generate/SKILL.md`; the deck skill currently lives at
  `.agents/skills/apollo/SKILL.md` until that rename lands. The deck workflow
...
- `docs/getcracked-inventory.md`: the durable topic backlog (Accepted in spec
  0006, not yet created).
